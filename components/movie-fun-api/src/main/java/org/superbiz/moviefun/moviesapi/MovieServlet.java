...
public class MovieServlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("Add".equals(action)) {
            ...
            int pageCount = (count / moviesPageSize);
            if (pageCount == 0 || count % moviesPageSize != 0) {
                pageCount++;
            }
            ...
            int start = (page - 1) * moviesPageSize;
            List<MovieInfo> range;

            if (StringUtils.isEmpty(key) || StringUtils.isEmpty(field)) {
                range = moviesClient.findAll(start, moviesPageSize);
            } else {
                range = moviesClient.findRange(field, key, start, moviesPageSize);
            }
